...
geodesic_cache_dir = os.path.join(
    base_path, "data", "geodesic_cache")  # for evaluating error metrics
            ...
            *shape2, name2 = shape2
            ...
            if with_geodesic_error:
                ...
                if not train:
                    ...
                    fname_lab1 = os.path.join(t_inj_path, "tr_reg_label{0}.vts".format(str(fidx).zfill(3)))
                    np.savetxt(fname_lab1, inj_label, fmt='%d')
                    ...
                    if fidx == 0:
                        ...
                        fname_lab2 = os.path.join(t_bij_path, "tr_reg_label{0}.vts".format(str(fidx).zfill(3)))
                        np.savetxt(fname_lab2, bij_label, fmt='%d')

                        bij_vts2on1 = bij_labels2to1[vts2]
                        bij_pair = diffusion_net.utils.toNP(bij_vts2on1)
                        fname_2 = os.path.join(t_bij_path, "tr_reg_{0}.vts".format(str(fidx).zfill(3)))
                        np.savetxt(fname_2, bij_pair, fmt='%d')
            ...
            if not train:
                ...
                fname_3r = os.path.join(t_prob_path, "prob_fmap_r{0}.vts".format(str(fidx).zfill(3)))
                fname_3c = os.path.join(t_prob_path, "prob_fmap_c{0}.vts".format(str(fidx).zfill(3)))
                np.savetxt(fname_3r, C_rprob, fmt='%1.3f')
                np.savetxt(fname_3r, C_cprob, fmt='%1.3f')
                if fidx == 0:
                ...
                fname_4 = os.path.join(t_bin_path, "bin_fmap_{0}.vts".format(str(fidx).zfill(3)))
