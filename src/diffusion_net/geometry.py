...
def find_bij_knn(points_source, points_target, k, largest=False, omit_diagonal=False, method='brute'):
    ...
    if method == 'brute':
        ...
        result = torch.topk(dist_mat, k=k, largest=largest, sorted=True)
        # for each result indices, if one index has same rank, pick one that has less values
        trans_dist = result.values.transpose(0, 1).contiguous()
        trans_idx = result.indices.transpose(0, 1).contiguous() # each row is the top i rank
        # print(trans_dist.size())
        # print(trans_idx.size())
        # for i in range(k):
        #     if all_unique(trans_idx[i], k) == False:
        #         print("{}: NO".format(i))
        #     else:
        #         print("{}: PASS".format(i))
        ...
        for i in tqdm([_n for _n in range(k)]):
            ...
            visited_idx = defaultdict(list) # idx: (distance, idx_on_bij, i=topk)
            for j in range(k):
            ...
            # 1
            # print("top{} len: {}".format(i, len(_key)))
                    ...
                    if len(visited_idx[elem]) == 1:
                        ...
                        for m in range(len(visited_idx[elem])):
                            ...
                            if bij[cur[1]] == -1: 
                                ...
                                bij[cur[1]] = elem
                                # print("after bij[{}]: {}".format(cur[1], bij[cur[1]]))
                                bij_dist[cur[1]] = cur[0]
                                r_visited.add(elem)
                                break
            # print("r_visited len:{}".format(len(r_visited)))
        ...
        print("Is label all unique? {}".format(all_unique(bij, k)))

        bij.unsqueeze(1)
        bij_dist.unsqueeze(1)

        # inds, dists = toNP(result.values), toNP(result.indices)
...
def get_prob_fmap(C):
    """
    Input: functional matrix C
    Output: C with element value between 0 and 1 (probability of mapping)
    0-0.5: normalized value between 0 < |C| <= 0.11
    0.5-1: normalized value between 0.11 < |C| < max_val of elem
    """
    # normalization
    C_abs = torch.abs(C)
    C_abs -= torch.min(C_abs)
    C_abs /= torch.max(C_abs)

    # each row's probability should sum up to 1
    r_1 = C_abs/C_abs.sum(axis=1, keepdim=True)
    c_1 = C_abs/C_abs.sum(axis=0, keepdim=True)

    # return C_abs, r_1, c_1
    return r_1, c_1
...
def get_bin_fmap(C):
    """
    Input: functional matrix C
    Output: C with 0 and 1 element
    1: absolute value larger than |C| > 0.11
    """
    C_abs = torch.abs(C)
    t_func = nn.Threshold(threshold=0.11, value=0)
